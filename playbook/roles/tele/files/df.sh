...
if [ "`df | grep "/dev/sda1" | awk '{print $5}' | sed 's/\%//'`" -ge 10 ]; 
 then echo "Disk usage exceeded 95%" | curl -s -X POST https://api.telegram.org/bot1493001337:AAEHNRl87mm47EfqY-d4b-_RWWcEvrZuANk/sendMessage -d chat_id=-469980725 -d text="Info - Тестовое уведомление. Диск заполнен больше чем на 10%!";
fi
